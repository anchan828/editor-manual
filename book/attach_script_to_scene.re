...
//image[ss02][空のゲームオブジェクトを生成せずにスクリプトをアタッチできる！]{
...
通常はシーンアセットにコンポーネントをアタッチすることはできません。また、エディター拡張によってシーンアセットにコンポーネント情報を追加することもできません。

なので今回は@<b>{見た目だけシーンアセットにコンポーネントがアタッチできる}仕組みを実装します。

=== シーンアセットに対応したプレハブを生成する

シーンにアタッチするコンポーネントは@<b>{自動生成されたプレハブ}に保持します。

特定のフォルダー（他のユーザーが触らないEditorフォルダー以下が好ましい）配下にプレハブを作成します。この時、プレハブ名をシーンアセットと関連するものにしますが、シーン名（パスも同様）は同名のものが存在する可能性があるので guid で管理します。
...
プレハブのコンポーネントを取得後 @<b>{コンポーネントに対応した Editor オブジェクトを生成}します。そして @<code>{Editor.OnInspectorGUI} を呼び出すことによって @<img>{ss05} のようなコンポーネントの表示を行うことができます。

//image[ss05][各コンポーネントのプロパティーが表示されている。いつも見る光景]{
...
これで @<img>{ss08} のような表示ができるようになりました。ですがまだシーンアセットにコンポーネントをアタッチすることができません。
