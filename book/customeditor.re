...
コンポーネントに直接アクセスする方法はとても楽な方法です。文字列でプロパティーにアクセスする SerializedObject と比べると、typo などケアレスミスも少なくなります。ですがこの方法で、エディター拡張を「きちんと」実装するには Undo 処理を実装しなければいけません。Undo は自動で登録されるものではなく、値を変更するときには Undo 処理を自前で実装します。対して、SerializedObject では Undo は自動で登録されるので Undo について気にする必要はありません。Undo の詳しい説明は @<chapref>{undo} をご覧ください。
