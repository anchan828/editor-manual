...
エディター拡張で何かを作成している場合、値を保存して次回以降も使いまわしたい時があります。その値はエディター拡張の設定に関するパラメーターであったり、ゲームに関係するパラメーターであったり様々です。Unity にはデータを保存する手段が大きく分けて 3 パターンあります。本章では、それらを紹介し目的に合った手段を選べるように解説していきます。
...
EditorPrefs で保存すべきものは@<b>{ウインドウの位置・サイズ}、@<b>{Unityエディターの環境設定（Preferencesにあるような設定）}の値です。EditorPrefs 経由で保存される値は全て平文で保存されます。決して、パスワードなど重要な情報は保存しないようにしてください。
...
EditorPrefs はメジャーバージョンごとに分けて保存されます。特に Windows はレジストリに値を保存します。EditorPrefs 経由「のみ」であれば問題はないのですが、直接レジストリを触ることも出来てしまうので、その過程で誤った設定をしてしまい、最悪 Windows が起動しなくなるということもありえます。十分気をつけてください。
...
また、ウインドウのサイズを保存する場合は、それほど値の重要性も高くないので OnDisable で値の保存をするのが適しています。決して OnGUI で毎回保存しないようにしてください。OnGUI の様な、多く呼び出されるメソッドで書き込み作業をやると高負荷となってしまいます。
...
様々なツールを使っていると、ログインのためのメールアドレスやパスワードが必要になってきたりします。Oauth のアクセストークンなどもその1つです。

EditorUserSettings.asset はバイナリ形式で保存されており簡単には中身を見れないようになっています。とはいっても Unity が提供している @<b>{binary2text} を使用することでバイナリをテキスト形式に変換し、見ることが出来てしまうので注意してください。
...
@<b>{プロジェクト内で共有できるデータの保存方法。}様々な応用が効く保存方法です。チーム内で共有したい設定や、大量のデータを保存したい場合にこの方法を選択します。
...
//image[ss02][インスペクターで値の編集を行うことが出来る]{
...
エディター拡張で作成したアセットのデータや設定ファイル、ビルド後にゲームデータとして使うデータベースとしての役割を持たせることが出来ます。

=== ScriptableObjectを保存する場所

Projectの Assets フォルダ以下であればどこにでも保存することが出来ます。エディター拡張専用の ScriptableObject であれば Editor フォルダ以下に作成するのが好ましいです。
