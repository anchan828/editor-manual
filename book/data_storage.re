...
EditorPrefs で保存すべきものは@<b>{ウィンドウの位置・サイズ}、@<b>{Unityエディターの環境設定（Preferencesにあるような設定）}の値です。独自アセットでも環境に関する設定であれば EditorPrefs を使用するようにしてください。注意として EditorPrefs 経由で保存される値は全て平文で保存されます。決して、パスワードなど重要な情報は保存しないようにしてください。
