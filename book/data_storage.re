...
@<b>{PC内で共有できるデータの保存方法です。}プロジェクトに縛られず、Unity エディターをまたいで値を共有するのに適しています。

=== 影響範囲

保存した値はメジャーバージョンごとのUnity エディターに影響します。
...
EditorPrefs で保存すべきものは@<b>{ウィンドウの位置・サイズ}、@<b>{Unity エディターの環境設定（Preferencesにあるような設定）}の値です。独自アセットでも環境に関する設定であれば EditorPrefs を使用するようにしてください。注意として EditorPrefs 経由で保存される値は全て平文で保存されます。決して、パスワードなど重要な情報は保存しないようにしてください。
...
//image[ss03][com.unity3d.UnityEditor5.x.plist を Xcode で開いた状態]{
