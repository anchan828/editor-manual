...
最も簡単にギズモを表示する方法として、2つのメソッドが用意されています。そのメソッドは MonoBehaviour の派生クラスで、 Update 関数のようなコールバックとして使用できます。@<b>{ゲームオブジェクトまたは親を選択中の時のみ表示する} @<code>{OnDrawGizmosSelected}、@<b>{ゲームオブジェクトがアクティブの時に常に表示する} @<code>{OnDrawGizmos} というように 2 つは動作が異なります。
