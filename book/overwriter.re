...
Unityでもこのようなダイアログを表示したいのですが、Unityにはこのようなファイルの上書き機能はありません。同名のファイルがドラッグ＆ドロップされた場合、ファイル名の最後に数字を付けて別ファイルとして扱われます。
...
ファイルの上書きをしたい場合は、Unity上ではなくFinder経由で上書きを行わなければなりません。しかし、Finder上で操作するのは、アセットが壊れてしまうかもしれない大きなリスクを抱えてしまいます。Unity エディターの管理外でアセットの変更を行うことは非推奨とされているからです。
...
ファイルを別物として扱わないようにするには.metaファイルも一緒に移動させれば良い話なのですが、Unity独特の仕様なため、.metaファイルを一緒に移動させるのを忘れがちです。（忘れたままUnity エディターに戻った場合、.metaファイルが自動で削除/生成されて悲惨なことに...）

===[/column]

ファイルの上書き問題をUnity上で解決したものが本章で作成する@<b>{Overwriter}です。
...
まずは、アセットの上書き処理に移るために、Unity エディターに何のアセットをインポートしようとしているか、何がインポートされたかを知らなければいけません。アセットのインポートに関する処理は @<b>{AssetPostprocessor} クラスを使うことで実装できます。
...
本章ではアセットを上書きすることを目的としていますが、処理的には「@<b>{別アセットとしてインポート}した後、置き換える」処理となります。これは Unity の仕様の関係で、インポート時に、同じ階層に同名のファイルがあった場合には@<b>{既にリネームした}状態でインポートしてしまうためです。例えば、hoge.png をインポートする時に、既に同名のアセットがあればhoge 1.png となります。
