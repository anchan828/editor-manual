...
ファイルの上書きをしたい場合は、Unity上ではなくFinder経由で上書きを行わなければなりません。しかし、Finder上で操作するのは、アセットが壊れてしまうかもしれない大きなリスクを抱えてしまいます。Unityエディターの管理外でアセットの変更を行うことは非推奨とされているからです。
...
まずは、アセットの上書き処理に移るために、Unityエディターに何のアセットをインポートしようとしているか、何がインポートされたかを知らなければいけません。アセットのインポートに関する処理は @<b>{AssetPostprocessor} クラスを使うことで実装できます。
