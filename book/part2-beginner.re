...
//lead{
エディター拡張を始める最初の段階として、知っておかなければならないことがあります。この章ではエディター拡張の世界へ入る第一歩としてエディター拡張で重要な「フォルダー」について説明します。
//}
...
UnityEditorでは @<b>{Assembly-CSharp-Editor.dll} を生成し、エディターAPIとランタイムAPIの住み分けを行うことでこの問題を解決しています。この Assembly-CSharp-Editor.dll はビルド時には含まれないのでビルドエラーも発生しません。@<b>{Assembly-CSharp-Editor.dll} は @<b>{Editor} フォルダー内のスクリプトファイルがコンパイルされて生成されます。
