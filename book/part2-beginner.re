...
//lead{
エディター拡張を行うためには知っておかなければならないことが2点あります。この章ではエディター拡張の世界へ入る第一歩としてエディター拡張で重要な「フォルダ」について説明します。
//}
...
EditorフォルダはエディターAPIを使用するための特別なフォルダです。
...
「Resources」フォルダのようにエディター拡張のみで使用するリソースを格納しておくフォルダです。「Editor Default Resources」フォルダ内にあるアセットは@<b>{EditorGUIUtility.Load}でアクセスすることが可能です。
