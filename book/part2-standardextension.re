...
//lead{
Unity エディターは、既に内部でエディター拡張の機能を使い、さまざまな拡張を行っています。ユーザーは属性（Attribute）を付けるだけで、それらの拡張にアクセスすることができ、自由にカスタマイズを行えます。この章では既に用意されているものを使ってみて「エディター拡張ではこういうことができる」ということを知りましょう。
//}
...
開発時には「なんとなくな値」で「それとない動き」を作ることはよくあります。それを普段のインスペクター表示で行うには、不向きなところが出てきます。それの解決策の1つとして@<b>{スライダー}で操作できるようにして操作性を上げる方法があります。
...
ゲーム再生中でなくても MonoBehaviour を継承したコンポーネントの 主要な関数が呼び出されるようになります。呼び出されるタイミングはゲームオブジェクトが更新された時です。シーンアセットをダブルクリックして、シーンをロードした時には Awake と Start 関数が、インスペクターなどでコンポーネントの変数などを変更したら Update 関数が呼び出されます。また、OnGUI で実装した GUI がエディターのGUI描画サイクルに則って常に表示されるようになります。
