...
通常で出来ないとなれば、エディタ拡張を使って拡張できる方法を模索することになります。

=== パーティクルを制御する Editor API は存在しない

残念ながらエディタ拡張用の API は用意されていません。ランタイム用の API を駆使して編集していくことになります。

=== API で全てのプロパティを触れるわけではない
...
=== Prefabを作成して、テキストエディタで見る

プロパティを知るうえで最も簡単な方法は、テキストベースで保存されたアセットをテキストエディタで見ることです。ParticleSystem のプロパティを知りたい場合は、ParticleSystem をアタッチしたゲームオブジェクトをプレハブにしましょう。
...
テキストエディタで直接、値を書き換えてもいいのですが Unity の管理外での変更となるため @<b>{Unity を起動したままテキストエディタで編集を行うとデータが破損してしまう}可能性があります。どうしてもテキストエディタで編集したい場合は @<b>{Unity を終了}させてから行うようにしてください。
...
通常の HorizontalSlider は、エディタ上では @<img>{ss07} のように、つまみの部分が小さなボタンに見えます。これでは小さすぎて扱いづらいかもしれません。そこで HorizontalSlider のスタイルをカスタマイズして見た目を大きくします。@<fn>{2}
