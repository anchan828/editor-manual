...
AnimationClipのインスペクターは3Dモデルのためのアニメーションが基準となっており、プレビュー画面も3Dアニメーションしか再生できず、2Dアニメーションでは使用できません。
...
もともとは、@<code>{AssetPreview.GetAssetPreview} はプロジェクトビューなどで表示するための機能です。プロジェクトビューではそこまで高解像度なテクスチャサイズは必要としていません。なので強制的に 128x128 へとリサイズされます。
